...
form_view_model.user_name.subscribe(() => {
    ...
    // ボタンの制御
    if (user_name_is_correct && password_is_correct && password_sub_is_correct) {
...
form_view_model.password.subscribe(() => {
    ...
    if (password !== password_sub) {
        // 文字制限
        form_view_model.password_sub_error_message("パスワードが一致しません");
        password_sub_is_correct = false;
    } else {
        form_view_model.password_sub_error_message("");
...
// パスワード（再入力）の判定
form_view_model.password_sub.subscribe(() => {
    // パスワード
    const password = form_view_model.password();
    // 再入力されたパスワード
    const password_sub = form_view_model.password_sub();
